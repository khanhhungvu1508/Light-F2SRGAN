...
def conv_same_padding(in_channels, out_channels, kernel_size, stride=1, dilation=1, groups=1, bias=True, padding_mode='zeros'):
    # Calculate the padding size needed to maintain the same height and width
    padding = (((stride - 1) + dilation * (kernel_size - 1)) // 2)
    return nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation=dilation, groups=groups, bias=bias, padding_mode=padding_mode)
    ...
    def __init__(self, in_channels, scale_factor):
        if (scale_factor == 2):
            super(UpsampleBlock, self).__init__(
                SeperableConv2d(in_channels, in_channels * 2**2, kernel_size=3),
                nn.PixelShuffle(2),
...
#         out = out.mul(0.1)
#         out += x
#         out = self.skip_ops.mul_scalar(out, 0.1)
#         out = out / 10
        out = torch.div(out, torch.tensor([10.0], dtype = x.dtype).to(x.device))
        out = out + x
        return out
...
class Generator(nn.Module):
    def __init__(self, in_channels: int = 3, num_channels: int = 64, num_blocks: int = 16, upscale_factor: int = 4):
        ...
        self.residual = nn.Sequential(
            *[ResidualBlock(num_channels, index) for index in range(num_blocks)]
        )
        self.upsampler = UpsampleBlock(num_channels, scale_factor=upscale_factor)
...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        fft_dim = (-2, -1)
        y = torch.fft.rfftn(x.type(torch.float32), dim=fft_dim, norm=self.fft_norm)
        # FFT Shift
        y = torch.fft.fftshift(y)
        weight = torch.view_as_complex(self.complex_weight)
        y = ComplexConv(y, weight.type(torch.complex64))
        # FFT IShift
...
class SpectralTransform(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.enable_lfu:
            ...
            device_run = torch.device("cuda" if xs.is_cuda else "cpu")

            if h % 2 == 1:
                h_zeros = torch.zeros(xs.shape[0], xs.shape[1], 1, xs.shape[3], dtype = x.dtype).to(device_run)
                xs = torch.cat((xs, h_zeros), dim=2)
            if w % 2 == 1:
                w_zeros = torch.zeros(xs.shape[0], xs.shape[1], xs.shape[2], 1, dtype = x.dtype).to(device_run)
