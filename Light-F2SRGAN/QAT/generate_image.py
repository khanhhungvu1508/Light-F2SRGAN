...
def F2SRGAN_QAT_psnr_testing(_path, model_path, ds):
    netG = model_quant.to(DEVICE)
...
UPSCALE_FACTOR = 4
RGB_RANGE = 255
MODEL_FILEPATH = "../../pretrain_weight/F2SRGAN_4x.pt"
MODEL_FILEPATH_QAT = "../../pretrain_weight/F2SRGAN_4x_QAT.pth.tar"

model_before_QAT = copy.deepcopy(Generator(upscale_factor=UPSCALE_FACTOR))
model_before_QAT.load_state_dict(torch.load(MODEL_FILEPATH)['model'])
model_before_QAT.eval()
...
for ds in dataset:
    psnr, ssim_val = F2SRGAN_QAT_psnr_testing("../../SR_testing_datasets/" + ds,
                                             MODEL_FILEPATH_QAT, 
                                             ds)
